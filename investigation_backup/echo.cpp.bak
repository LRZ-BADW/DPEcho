...
int main(int argc, char** argv ) {
  ...
  int *nanInfo = malloc_shared<int>(5, qDev); // [vId, packed(which|fld), myDir, flag, stencilCI]
  for (int i = 0; i < 5; ++i) nanInfo[i] = -1;
  ...
  while( (problem.t() <= problem.tMax()) && (problem.iStep() < problem.nStep()) ){
	...
	qDev.parallel_for(getMatchingNdRange(rFlx, range<3>(wgMax,wgMax,wgMax)), maxReduction, [=](nd_item<3> it, auto &max) {
          ...
          // NaN detection: check inputs BEFORE holibRec, outputs BEFORE & AFTER physicalFlux
          sycl::atomic_ref<int, sycl::memory_order::relaxed, sycl::memory_scope::device> refN3(nanInfo[3]);
          if (refN3.load() < 0) {
            int which = 0, badFld = -1, stencilCi = -1;
            // Check v input cells that holibRec reads (stencil at vId..vId+3*vSt)
            for (int j = 0; j < REC_TOTAL_POINTS && !which; ++j) {
              int ci = vId + vSt * (j - REC_LEFT_OFFSET);
              for (int f = 0; f < FLD_TOT && !which; ++f) {
                if (v[f][ci] != v[f][ci]) { which = 128; badFld = f; stencilCi = ci; }
              }
            }
            // Check reconstructed states and flux outputs
            for (int i = 0; i < FLD_TOT && !which; ++i) {
              if (vL[i] != vL[i])        { which = 1;  badFld = i; }
              else if (vR[i] != vR[i])   { which = 2;  badFld = i; }
              else if (fL[i] != fL[i])   { which = 4;  badFld = i; }
              else if (fR[i] != fR[i])   { which = 8;  badFld = i; }
              else if (uL[i] != uL[i])   { which = 16; badFld = i; }
              else if (uR[i] != uR[i])   { which = 32; badFld = i; }
              else if (f[i][fId]!=f[i][fId]) { which = 64; badFld = i; }
            }
            if (which) {
              int exp3 = -1;
              if (refN3.compare_exchange_strong(exp3, myDir)) {
                nanInfo[0] = vId;  nanInfo[1] = (badFld<<8)|which;  nanInfo[2] = myDir;
                if (stencilCi >= 0) nanInfo[4] = stencilCi;
              }
            }
          }
        ...
        // NaN diagnostic: read nanInfo after each direction's flux kernel (prints once on first detection)
        if (problem.iStep() >= 63 && nanInfo[3] >= 0 && nanInfo[3] < 10) {
          int rank = 0; MPI_Comm_rank(MPI_COMM_WORLD, &rank);
          int x = nanInfo[0] % 40, y = (nanInfo[0] / 40) % 40, z = nanInfo[0] / 1600;
          int which = nanInfo[1] & 0xFF, fld = nanInfo[1] >> 8;
          const char *wstr = which==1?"vL":which==2?"vR":which==4?"fL":which==8?"fR":
                             which==16?"uL":which==32?"uR":which==64?"fHLL":which==128?"vIn":"?";
          if (which == 128) {
            int cx = nanInfo[4] % 40, cy = (nanInfo[4] / 40) % 40, cz = nanInfo[4] / 1600;
            fprintf(stderr, "nanFlux step=%d dir=%d vId=%d (x=%d y=%d z=%d) src=%s fld=%d stencilCell=(%d,%d,%d) rank=%d\n",
              problem.iStep(), nanInfo[2], nanInfo[0], x, y, z, wstr, fld, cx, cy, cz, rank);
          } else {
            fprintf(stderr, "nanFlux step=%d dir=%d vId=%d (x=%d y=%d z=%d) src=%s fld=%d rank=%d\n",
              problem.iStep(), nanInfo[2], nanInfo[0], x, y, z, wstr, fld, rank);
          }
          nanInfo[3] = 10; // Mark printed (keep >= 0 so guard blocks kernel re-detection)
        }
	...
	problem.dump(debug, grid        , "debug"                     );
      ...
      if (problem.iStep() >= 40) {
        for (int i = 0; i < Ncell; ++i) {
          bool duNaN = false;
          for (int f = 0; f < FLD_TOT; ++f) { if (du[f][i] != du[f][i]) { duNaN = true; break; } }
          if (duNaN) {
            fprintf(stderr, "duNaN step=%d ci=%d: rh=%s vx=%s vy=%s vz=%s bx=%s by=%s bz=%s pg=%s\n",
              problem.iStep(), i,
              du[RH][i]!=du[RH][i]?"nan":"ok", du[VX][i]!=du[VX][i]?"nan":"ok",
              du[VY][i]!=du[VY][i]?"nan":"ok", du[VZ][i]!=du[VZ][i]?"nan":"ok",
              du[BX][i]!=du[BX][i]?"nan":"ok", du[BY][i]!=du[BY][i]?"nan":"ok",
              du[BZ][i]!=du[BZ][i]?"nan":"ok", du[PG][i]!=du[PG][i]?"nan":"ok");
            break;
          }
        }
      }
    ...
    qDev.wait_and_throw();
    // Host-side NaN check: find first AND last NaN indices
    int firstNan = -1, lastNan = -1;
    for (int i = 0; i < Ncell; ++i) {
      if (v[VX][i] != v[VX][i]) {
    ...
    if (firstNan >= 0 && problem.iStep() <= 70) {
      for (int ci : {firstNan, lastNan}) {
        int x = ci / 1600, y = (ci % 1600) / 40, z = ci % 40;
        int globalX, globalY, globalZ;
        // We can't get rank info easily, just print local coords
        fprintf(stderr, "NaN at step=%d ci=%d (x=%d y=%d z=%d): rh=%s vx=%s vy=%s vz=%s bx=%s by=%s bz=%s pg=%.6e\n",
          problem.iStep(), ci, x, y, z,
          v[RH][ci]!=v[RH][ci]?"nan":"ok", v[VX][ci]!=v[VX][ci]?"nan":"ok",
          v[VY][ci]!=v[VY][ci]?"nan":"ok", v[VZ][ci]!=v[VZ][ci]?"nan":"ok",
          v[BX][ci]!=v[BX][ci]?"nan":"ok", v[BY][ci]!=v[BY][ci]?"nan":"ok",
          v[BZ][ci]!=v[BZ][ci]?"nan":"ok", v[PG][ci]);
      }
    }
