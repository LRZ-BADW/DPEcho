...
cd "$ROOT"
mpirun -np 1 "$bin" tests/par/grmhd_periodic.par > "$DIR/out/grmhd_serial.out" 2> "$DIR/out/grmhd_serial.perf"

extract_dt()    { grep -oP 'dt \K[0-9.eE+\-]+' "$1" 2>/dev/null; }
extract_mcups() { awk '{print $NF}' "$1" 2>/dev/null; }

R_OUT="$DIR/ref/grmhd_serial.out";  G_OUT="$DIR/out/grmhd_serial.out"
R_PERF="$DIR/ref/grmhd_serial.perf"; G_PERF="$DIR/out/grmhd_serial.perf"
