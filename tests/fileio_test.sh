...
cd "$ROOT"
mpirun -np 1 "$bin" tests/par/grmhd_fileio.par > "$DIR/out/fileio_test.out" 2> "$DIR/out/fileio_test.perf"

extract_dt()    { grep -oP 'dt \K[0-9.eE+\-]+' "$1" 2>/dev/null; }
extract_mcups() { awk '{print $NF}' "$1" 2>/dev/null; }

R_OUT="$DIR/ref/fileio_test.out";  G_OUT="$DIR/out/fileio_test.out"
R_PERF="$DIR/ref/fileio_test.perf"; G_PERF="$DIR/out/fileio_test.perf"
