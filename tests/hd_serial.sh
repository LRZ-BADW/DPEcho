...
if [[ -n "$bin" ]]; then
  ...
  cmake -S "$ROOT" -B "$BLD" -DCMAKE_CXX_COMPILER=mpiicpx -DCMAKE_BUILD_TYPE=Release -DSYCL_DEVICE=DEF -DMETRIC=CARTESIAN -DNRK=3 -DPHYSICS=HD 2>&1 | tail -1
  cmake --build "$BLD" -j"$(nproc)" 2>&1 | tail -1
fi
bin=$(find "$BLD" -maxdepth 1 -name 'dpecho_*' -type f -executable | head -1)

cd "$ROOT"
mpirun -np 1 "$bin" tests/par/hd_periodic.par > "$DIR/out/hd_serial.out" 2> "$DIR/out/hd_serial.perf"

extract_dt()    { grep -oP 'dt \K[0-9.eE+\-]+' "$1" 2>/dev/null; }
extract_mcups() { awk '{print $NF}' "$1" 2>/dev/null; }

R_OUT="$DIR/ref/hd_serial.out";  G_OUT="$DIR/out/hd_serial.out"
R_PERF="$DIR/ref/hd_serial.perf"; G_PERF="$DIR/out/hd_serial.perf"
